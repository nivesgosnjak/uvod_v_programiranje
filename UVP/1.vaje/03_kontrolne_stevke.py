...
def dodaj_kontrolno_stevko(sklic):
    ...
    s11 = sklic//(10**10)%10
    s12 = sklic//(10**11)%10
    stevilo= s1*2 +s2*3 +s3*4+ s4*5 +s5*6+s6*7+s7*8+s8*9+s9*10+s10*11+s11*12+s12*13
    ostanek= stevilo%11
    a=11-ostanek
    if a<10:
        return sklic*10 +a
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjQsInVzZXIiOjEwNjc2fQ:1tmuqQ:2gkkAoFKIvmoAOYf4Hns_Hlb9XYZ6e89yFZoIeH4uLQ"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjUsInVzZXIiOjEwNjc2fQ:1tmuqQ:i0t5Bc2jXHm2WRASq5qnULu6I_4L9owBQWiZJuReBzI"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjYsInVzZXIiOjEwNjc2fQ:1tmuqQ:vfMtFSQH77sUxm30QKcOQ8yEZiqrKtM1pJKKV-AgCmY"
        try:
