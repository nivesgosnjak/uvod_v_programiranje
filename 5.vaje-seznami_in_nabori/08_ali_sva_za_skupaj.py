...
def razbij_na_stevke(stevilo):
    razbito=[]
    for n in range(len(str(stevilo))):
        razbito.append(int(str(stevilo)[n]))
    return razbito
# =====================================================================@013414=
# 2. podnaloga
...
def prestej_crke(geslo, niz):
    ...
    niz=niz.upper()
    geslo=geslo.upper()
    for n in range(len(geslo)):
        presteto.append(niz.count(geslo[n]))
    return presteto
...
def sestej_stevke(stevke):
    vsota=[]
    for n in range(len(stevke)-1):
        a= stevke[n] + stevke[n+1]
        vsota +=razbij_na_stevke(a)
    return vsota
# =====================================================================@013416=
# 4. podnaloga
...
def ujemanje(oseba1, oseba2, geslo="LOVES"):
    crke= prestej_crke(geslo,oseba1+oseba2)
    while len(crke)>2:
        crke= sestej_stevke(crke)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzQxMywidXNlciI6MTA2NzZ9:1twQd5:DbMIo4fNV2AaZrY2AVv8XHay_9RRW_tmXGbBl-Wsouk"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzQxNCwidXNlciI6MTA2NzZ9:1twQd5:x7OGPRMYD-x7YNY8a9r2i3fXMyR_sEwLD75AxJ6IXK8"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzQxNSwidXNlciI6MTA2NzZ9:1twQd5:ug62gex16XxnRtDlh8O8V3HjHdzufl7LqRkzo4WmvMM"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMzQxNiwidXNlciI6MTA2NzZ9:1twQd5:FJN4lBIoyh678LDR6Wand4HOia7cGI9u3qEAk3JKpTM"
        try:
